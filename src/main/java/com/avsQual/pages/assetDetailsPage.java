...
public class assetDetailsPage extends BaseClass {
                ...
                WebElement AVSappClose_Btn = null;
...
// Check the presence of AssetHub_ImgHldr field
                ...
                public boolean setup_Visible()
                {
                                WebElement setup = driver.findElementByClassName("ListViewItem");
                                return IsElementVisibleStatus(setup);
                }
                public boolean is_SetupVisible()
                {
                                boolean status = false;
                                try
                                {
                                WebElement setup = driver.findElementByClassName("ListViewItem");
                                if(IsElementVisibleStatus(setup))
                                {
                                	status = true;
                                }
                                }
                ...
                //Verifying Qualifications Count is visible or not 
                public int QualificationCnt()
                {
                ...
                {
                	WebElement rprtCnt = driver.findElementByAccessibilityId("ReportsCountTextBlock");
                	String value = FetchText(rprtCnt);
                	int count = Integer.parseInt(value);
                	return count;
                }
                
//displayed setup correct or not
                public String setup_DispCorrect(String sname)
                {
                                WebElement name = driver.findElementByName(sname);
...
// Check the presence of Sop Number Field
                public boolean IsSOPNumberField_Editable() {
...
// Check the presence of Run Number Text Box
                public boolean IsRunNumberField_Editable() {
...
// Enter Value into Run Number Text Box
                public void clear_RunNumber() throws InterruptedException {
                                WebElement Run_Field = driver.findElementByAccessibilityId("RunNumberTextBox");
                                clickOn(Run_Field);
                                ClearText(Run_Field);
                }             
...
// Click on OK button for SOP/Run creation
                public SelectAVSPage StartQual_OKPopUp(String mon, String date, String yr, String hours, String mins, String secs) throws IOException, InterruptedException {
                    WebElement OK_Btn_InSOPBlock = driver.findElementByAccessibilityId("StartQualificationOKButton");
                    clickOn(OK_Btn_InSOPBlock);
                   try
                   {
                    if(is_PromptQualificationStartDateTimePopup()==true)
                    {
                    	Thread.sleep(1000);
                ...
                public SelectAVSPage StartQual_OKPopUp1(String hours, String mins, String secs) throws IOException, InterruptedException {
                    WebElement OK_Btn_InSOPBlock = driver.findElementByAccessibilityId("StartQualificationOKButton");
                    clickOn(OK_Btn_InSOPBlock);
                   try
                   {
    ...
    //                	selectReqDate(mon, date, yr);
                ...
                public boolean is_PromptQualificationStartDateTimePopup()
                {
                	WebElement timePopup = driver.findElementByAccessibilityId("PromptQualificationStartDateTimePopup");
                	boolean status = false;
                	try
                	{
                		IsElementVisibleStatus(timePopup);
							status = true;
                	}
                	catch (Exception e) {
						status = false;
...
//                }
                ...
                public SelectAVSPage StartQual_OKButton() throws IOException, InterruptedException {
                    WebElement OK_Btn_InSOPBlock = driver.findElementByAccessibilityId("StartQualificationOKButton");
                    clickOn(OK_Btn_InSOPBlock);
                    Thread.sleep(1000);
                    return new SelectAVSPage();
                }
                public void click_StartQual_OKButton() throws IOException, InterruptedException {
                    WebElement OK_Btn_InSOPBlock = driver.findElementByAccessibilityId("StartQualificationOKButton");
                    clickOn(OK_Btn_InSOPBlock);
                }
                
                //Verifying the SOP/Protocol popup will displayed or not
                public boolean isSOPPopUp_Displayed()
                {
                	WebElement SOPPopup = driver.findElementByAccessibilityId("StartQualificationOKButton");
                	boolean status = false;
                	try
                	{
                		IsElementVisibleStatus(SOPPopup);
                		status = true;
                	}
                	catch (Exception e) {
                		status = false;
...
//dispalyMessageTextBlock without privileges
                public String popuptxt_WithoutPrivileges()
                {
                WebElement ele = driver.findElementByAccessibilityId("dispalyMessageTextBlock");
...
// Check the presence of Asset create button
...
// Click on Copy asset button
                ...
                {
                	List<WebElement> listItem = driver.findElementByAccessibilityId("AssetCyclesGridView").findElements(By.className("ListViewItem"));
                	clickOn(listItem.get(0));
                	...
                	for(int i=0; i<=listItem.size(); i++)
                	{
                		...
                		try
                		{
                			WebElement sName = driver.findElementByAccessibilityId("AssetCyclesGridView").findElement(By.name(SName));
                			if(FetchText(sName).equals(SName))
                			{
                				clickOn(sName);
                				break;
                				}
                ...
                clickOn(listItem.get(0));
                	...
                	for(int i=0; i<=listItem.size(); i++)
                	{
                		...
                		try
                		{
                			WebElement sName = driver.findElementByAccessibilityId("AssetCyclesGridView").findElement(By.name(SName));
                		//	if(FetchText(sName).equals(SName))
                			if(IsElementVisibleStatus(sName))
                			{
                				clickOn(sName);
                				status = true;
                				break;
...
// click on the Print_Button
                ...
                //get setup name from the list
                public String get_SetupName(String str)
                {
                	//List<WebElement> setupName = driver.findElementByAccessibilityId("AssetCyclesGridView")
                	//List<WebElement> setupName = driver.findElementByAccessibilityId("ListViewItem")
                	List<WebElement> setupName = driver.findElementByName("NextGenValidator.Infrastructure.DataModel.AssetDetailCyclesDataSource")
                			.findElements(By.className("TextBlock"));
                	String name = "";
                	for(WebElement sName:setupName)
                	{
                		name = FetchText(sName);
                		break;
                	}
               // 	name = FetchText(setupName.get(index));
...
// Select any Doc file and click on that
                public void Select_DocFile(String DC) throws InterruptedException, IOException {
                                List<WebElement> DOCList = driver.findElementByName("NextGenValidator.Infrastructure.DataModel.AssetDetailCyclesDataSource")
                                                                .findElements(By.className("TextBlock"));
                ...
                public Copyassetpage clickon_Copyasset() throws InterruptedException, IOException {

                    WebElement copyAsset=    driver.findElementByAccessibilityId("CopyAssetsButton");
...
// Check on-click functionality of PDF icon for Detailed report under Reports tile-Qualifications sub tab
                ...
                public int setup_Count() {
                	List<WebElement> SetupList = driver.findElementByAccessibilityId("AssetCyclesGridView")
                                                      .findElements(By.name("NextGenValidator.Infrastructure.DataModel.AssetDetailCyclesDataSource"));
                      return SetupList.size();
      }
                //Verify Wether Setups exists or not
                public boolean is_SetupDisplayed(String SName)
                {
             //   	List<WebElement> setupList = driver.findElementByAccessibilityId("AssetCyclesGridView").findElements(By.name(SName));
          ...
          /*      	boolean status = false;
                	for(WebElement sList:setupList)
                	{
                		try
                		{
                			if(FetchText(sList).equalsIgnoreCase(SName))
                			{
                				status = true;
                			}
                		}
